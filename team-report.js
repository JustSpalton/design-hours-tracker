...
function renderProductTotals(){
  ...
  panel.innerHTML=`<div class="product-totals-head"><div><h3>Job type totals — everyone</h3><span>${escapeHtml(typeof rangeLabel==='function'?rangeLabel():`W/C ${fmtDate(selectedWeek)}`)}</span></div></div><div class="product-total-grid">${hasData?products.map(p=>`<div class="product-total"><div class="product-total-label">${escapeHtml(p)}</div><div class="product-total-value">${Number(totals[p]||0)}</div><div class="product-total-sub">jobs • ${hasHours?`${Number(hours[p]||0).toFixed(1)} design hours`:'hours not imported'}</div></div>`).join(''):'<div class="breakdown-empty">No job type breakdown data for this period yet.</div>'}</div>`;
}
...
function renderManagerReport(){
  ...
  <section class="report-block"><h3>Job type totals — everyone</h3><div class="report-chips">${productNames.length?productNames.map(p=>`<div><span>${escapeHtml(p)}</span><strong>${Number(products[p]||0)} jobs</strong><em>${hasProductHours?`${Number(productHours[p]||0).toFixed(1)} design hours`:'hours not imported'}</em></div>`).join(''):'<p>No job type breakdown data imported for this period.</p>'}</div></section>
  <section class="report-block"><h3>Work category totals — everyone</h3><div class="report-chips">${statusNames.length?statusNames.map(s=>`<div><span>${escapeHtml(s)}</span><strong>${Number(statuses[s]||0)} jobs</strong><em>${hasStatusHours?`${Number(statusHours[s]||0).toFixed(1)} design hours`:'hours not imported'}</em></div>`).join(''):'<p>No work category breakdown data imported for this period.</p>'}</div></section>
  ${breakdownCoverage<weeks.length?`<div class="report-note">Job-type and work-category figures cover ${breakdownCoverage} of ${weeks.length} week${weeks.length===1?'':'s'} in this report. Re-import older estimator spreadsheets to fill any missing breakdown history.</div>`:''}
